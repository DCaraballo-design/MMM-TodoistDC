...
Module.register("MMM-TodoistDC", {
	...
	start: function () {
		...
		this.userPresence = true; // PIR / user presence: true by default so no impact for users without MMM-PIR-Sensor
	...
	notificationReceived: function (notification, payload) {
		if (notification === "USER_PRESENCE") { // notification sended by module MMM-PIR-Sensor. See its doc
			...
			this.userPresence = Boolean(payload);
			this.GestionUpdateIntervalToDoIst();
		}
	},
	...
	filterTodoistData: function (tasks) {
		...
		if (tasks.items === undefined || tasks.items === null) {
			return;
		}
